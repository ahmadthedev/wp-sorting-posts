<div id="sortby"> SORT BY: &nbsp;
  <select class="dropdown-class" name="sort-posts" id="sortbox" onchange="document.location.search=this.options[this.selectedIndex].value;">
    ...
    <option <?php if( isset($_GET["orderby"]) && trim($_GET["orderby"]) == 'post_views_count' && isset($_GET["order"]) && trim($_GET["order"]) == 'ASC' ){ echo 'selected'; } ?> value="?orderby=post_views_count&order=ASC">Views Asc</option>
    <option <?php if( isset($_GET["orderby"]) && trim($_GET["orderby"]) == 'post_views_count' && isset($_GET["order"]) && trim($_GET["order"]) == 'DESC' ){ echo 'selected'; } ?> value="?orderby=post_views_count&order=DESC">Views Desc</option>
...
<?php
  ...
  if( $_GET['orderby'] != '' ) {
    if ( $_GET['orderby'] === 'post_views_count' ) {
      $args['meta_key']	=	'post_views_count'; // Set meta value
